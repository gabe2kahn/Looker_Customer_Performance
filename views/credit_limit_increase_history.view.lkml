view: credit_limit_increase_history {
  ...
  dimension: time_since_cl_increase {
    type: number
    sql: DATEDIFF(DAYS,${cl_change_date},current_date) ;;
  }
  ...
  measure: average_time_since_last_cl_increase {
    type: number
    sql: DATEDIFF(DAYS,AVG(CASE WHEN ${cl_change_recency} = 1 THEN ${cl_change_date}::date END),${snapshot_pt.snap_date}::date) ;;
